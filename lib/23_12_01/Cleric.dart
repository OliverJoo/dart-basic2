...
class Cleric {
  ...
  // selfAid 라는 마법 : MP를 5소비로 자신의 HP 를 최대 HP 까지 회복
  // 이 메소드는 인수가 없고, 리턴 값도 없다
  void selfAid() {
    if (mp >= 5) {
      mp -= 5;
      hp = maxHp;
  ...
  // pray 메소드는 인수에 “기도할 시간(초)"를 지정할 수 있고,
  // 리턴 값은 “실제로 회복된 MP 양" 을 반환
  // 회복량은 기도한 시간(초)에 랜덤하게 0 ~ 2포인트의 보정(3초 기도에 회복량은 3~5 포인트)
